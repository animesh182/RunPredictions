...
from PredictionFunction.PredictionTypes.daily_data_prep import prepare_data as prepare_daily_data
from PredictionFunction.PredictionTypes.alcohol_mix_data_prep import product_mix_predictions as prepare_alcohol_data
from PredictionFunction.PredictionTypes.hourly_data_prep import hourly_sales as prepare_hourly_data
from PredictionFunction.PredictionTypes.take_out_data_prep import type_predictor as prepare_takeout_data
...
async def main(mytimer: func.TimerRequest) -> None:
    ...
    for instance in data:
        ...
        if prediction_category=="hour":
            merged_data,historical_data,future_data = prepare_hourly_data(company,restaurant,start_date,end_date)
        elif prediction_category=="type":
            merged_data,historical_data,future_data = prepare_takeout_data(company,restaurant,start_date,end_date)
        ...
        model,future_df,current_df,event_holidays = restaurant_func(prediction_category,restaurant,merged_data,historical_data,future_data)
        forecast = predict(model,future_df,current_df,company,restaurant,start_date,end_date,prediction_category)
        new_df = current_df[["ds", "y"]].rename(columns={"ds": "ds", "y": "historical_sale"})
        forecast = pd.merge(
